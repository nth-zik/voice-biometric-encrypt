...
def float_to_bin_with_sign(value):
    """Chuyển đổi float sang chuỗi nhị phân theo chuẩn IEEE 754, kèm dấu."""
    sign_bit = '0' if value >= 0 else '1'  # 0 cho dương, 1 cho âm
    abs_value = abs(value)
    return sign_bit + format(abs_value, "063b")

def bin_with_sign_to_float(bin_str):
    """Chuyển đổi chuỗi nhị phân có dấu về giá trị float."""
    sign_bit = bin_str[0]  # Lấy dấu từ bit đầu tiên
    value_bin = bin_str[1:]  # Lấy phần giá trị từ các bit còn lại
    # value = struct.unpack(">d", struct.pack(">Q", int(value_bin, 2)))[0]
    value = int(value_bin, 2)
    return value if sign_bit == '0' else -value

def binary_with_sign_to_embedding(binary_string, original_shape):
    """Chuyển đổi chuỗi nhị phân có dấu về lại embedding (dạng float)."""
    n = 64  # 1 bit dấu + 64 bit cho giá trị
    values = [bin_with_sign_to_float(binary_string[i:i+n]) for i in range(0, len(binary_string), n)]
...
def main():
    ...
    integer_representation = (embedding_np * DECIMAL_KEEP).astype(int)
    print(f'integer_representation {integer_representation}')
    ravel_length = len(integer_representation.ravel())
    print(integer_representation.ravel())
    # Chuyển mỗi giá trị float sang nhị phân
    binary_representation = "".join(
        float_to_bin_with_sign(x) for x in integer_representation.ravel()
    )
    ...
    binary_string = bin(int.from_bytes(decrypted_data, byteorder="big"))[2:]
    # original_integer_representation = np.array(
    #     [int(binary_string[i : i + 64], 2) for i in range(0, len(binary_string), 64)]
    # )
    # print(original_integer_representation)
    # Chuyển đổi số nguyên về lại float

    # original_abs_embedding_np = (
    #     original_integer_representation.astype(float) / DECIMAL_KEEP
    # )
    original_embedding_np = binary_with_sign_to_embedding(binary_representation, stored_original_shape)
    print(original_embedding_np)
