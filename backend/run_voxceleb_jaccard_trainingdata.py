...
def main():
    ...
    data_dir = "./test_data/voxceleb_train/wav/"  # Thay bằng đường dẫn tới thư mục VoxCeleb1 của bạn
    trial_file = "./trials/list_test_hard_fromtrain.txt"  # Đường dẫn tới tệp veri_test.txt
    ...
    embeddings_file = "./embeddings_list_test_hard_fromtrain.txt"
    ...
    with open("all_embeddings_list_test_hard_fromtrain.txt", "w") as f:
    ...
    if len(scores_cosine) > 0:
        eer_cosine, threshold = compute_eer(labels_cosine, scores_cosine)
    ...
    for exponent in range(1, 8):
        ...
        hex_embeddings_file = (
            f"./hex_embeddings/hex_embeddings_jaccard_new_list_test_hard_fromtrain_{exponent}.json"
        )
        ...
        if len(scores) > 0:
            ...
            print(
                f"EER based on Jaccard Distance with DECIMAL_KEEP=10^{exponent}: {eer * 100:.2f}%"
            )
            eer_results.append(
                {
                    "DECIMAL_KEEP": exponent,
                    "EER_Jaccard": eer * 100,
