...
def dice_similarity(hex1, hex2):
    """Tính độ tương đồng Sorensen-Dice giữa hai chuỗi hexadecimal."""
    bin1 = bin(int(hex1, 16))[2:]
    bin2 = bin(int(hex2, 16))[2:]
    ...
    # Tính giao và tổng số bit 1
    intersection = sum(c1 == c2 == '1' for c1, c2 in zip(bin1, bin2))
    total_1s = bin1.count('1') + bin2.count('1')

    # Tránh chia cho 0 bằng cách trả về 0 nếu cả hai chuỗi không có bit 1
    if total_1s == 0:
        return 0.0
    else:
        return 2 * intersection / total_1s
...
def overlap_coefficient(hex1, hex2):
    """Tính Overlap Coefficient giữa hai chuỗi hexadecimal."""
    bin1 = bin(int(hex1, 16))[2:]
    bin2 = bin(int(hex2, 16))[2:]
    ...
    # Tính phần giao và độ dài của mỗi chuỗi
    intersection = sum(c1 == c2 == '1' for c1, c2 in zip(bin1, bin2))
    min_length = min(bin1.count('1'), bin2.count('1'))
...
def composite_similarity(hex1, hex2, alpha=0.5):
    """Tính điểm tương đồng kết hợp giữa Jaccard và Dice Similarity."""
    jaccard = jaccard_similarity(hex1, hex2)
    dice = dice_similarity(hex1, hex2)
    return alpha * jaccard + (1 - alpha) * dice

def tanimoto_similarity(hex1, hex2):
    """Tính Tanimoto Similarity giữa hai chuỗi hexadecimal."""
    bin1 = bin(int(hex1, 16))[2:]
    bin2 = bin(int(hex2, 16))[2:]
    ...
    # Tính phần giao và tổng hợp phần tử
    intersection = sum(c1 == c2 == '1' for c1, c2 in zip(bin1, bin2))
    union = bin1.count('1') + bin2.count('1') - intersection

    if union == 0:
        return 0.0
    else:
        return intersection / union
def ochiai_coefficient(hex1, hex2):
    """Tính Ochiai Coefficient giữa hai chuỗi hexadecimal."""
    bin1 = bin(int(hex1, 16))[2:]
    bin2 = bin(int(hex2, 16))[2:]
    ...
    # Tính phần giao và tích số lượng bit 1 của mỗi chuỗi
    intersection = sum(c1 == c2 == '1' for c1, c2 in zip(bin1, bin2))
    len_A = bin1.count('1')
    len_B = bin2.count('1')
    ...
    if len_A == 0 or len_B == 0:
        ...
        return intersection / (len_A ** 0.5 * len_B ** 0.5)

def kulczynski_similarity(hex1, hex2):
    """Tính Kulczynski Similarity giữa hai chuỗi hexadecimal."""
    bin1 = bin(int(hex1, 16))[2:]
    bin2 = bin(int(hex2, 16))[2:]
    ...
    # Tính phần giao và tổng số bit 1 của mỗi chuỗi
    intersection = sum(c1 == c2 == '1' for c1, c2 in zip(bin1, bin2))
    len_A = bin1.count('1')
    len_B = bin2.count('1')
    ...
    if len_A == 0 or len_B == 0:
        ...
        return 0.5 * ((intersection / len_A) + (intersection / len_B))

def sokal_michener_similarity(hex1, hex2):
    """Tính Sokal-Michener Similarity giữa hai chuỗi hexadecimal."""
    bin1 = bin(int(hex1, 16))[2:]
    bin2 = bin(int(hex2, 16))[2:]
    ...
    # Tính phần giao của các bit 1 và các bit 0
    intersection_1 = sum(c1 == c2 == '1' for c1, c2 in zip(bin1, bin2))
    intersection_0 = sum(c1 == c2 == '0' for c1, c2 in zip(bin1, bin2))
    total_bits = len(bin1)  # Tổng số bit

    # Tính Sokal-Michener Similarity
    return (intersection_1 + intersection_0) / total_bits
...
def main():
    ...
    for exponent in range(1, 8):  # Từ 8 đến 15
        ...
        hex_embeddings_file = f"./hex_embeddings_dice_{exponent}.json"
        ...
        for file_path in tqdm(file_list):
            ...
            # embedding_quantized = quantize_embedding(embedding_np)
        ...
        for label, path1, path2 in tqdm(trials):
            if path1 in hex_embeddings and path2 in hex_embeddings:
                ...
                similarity = sokal_michener_similarity(hex1, hex2)
