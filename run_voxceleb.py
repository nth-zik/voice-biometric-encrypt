...
def quantize_embedding(embedding_np, num_levels=256):
    ...
    # Chuẩn hóa embedding vào khoảng [0, 1]
    min_val = np.min(embedding_np)
    max_val = np.max(embedding_np)
    embedding_normalized = (embedding_np - min_val) / (max_val - min_val)
    # Lượng tử hóa thành num_levels mức
    embedding_quantized = np.floor(embedding_normalized * (num_levels - 1)).astype(int)
    return embedding_np
    ...
    for exponent in range(1, 8):  # Từ 8 đến 15
        ...
        for file_path in tqdm(file_list):
            ...
            binary_representation = embedding_to_gray_with_sign(
                embedding_quantized.ravel(), decimal_keep
            )
