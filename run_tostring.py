...
def main():
    ...
    embedding = extract_speaker_embd(
            model,
            fn='test_data/00001.wav',
            n_samples=n_samples,
            n_segments=n_segments,
            gpu=gpu,
        )
    # print(embedding)
    b = embedding.cpu().numpy()
    # print(b)
    
    ts = b.tolist()
    ts = ast.literal_eval(str(ts))
 
    npa = np.asarray(ts, dtype=np.float32)
...
def extract_speaker_embd(model, fn: str, n_samples: int, n_segments: int = 10, gpu: bool = False) -> np.ndarray:
